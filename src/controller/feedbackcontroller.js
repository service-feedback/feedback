...
let arr = ["Excellent", "Very Good", "Good", "Average", "Poor", ""]
        ...
        if (existingData) {
            ...
            for (const key in data) {
                if (keyValuesMap[key] !== undefined && !keyValuesMap[key].includes(data[key])) {
                    return res.status(400).send({
                        status: false,
                        message: `Invalid value "${data[key]}" provided for key "${key}". Valid values are: ${keyValuesMap[key].join(', ')}.`,
                    });
                }
            }
...
let lastWeekStartDate = moment().subtract(1, 'weeks').startOf('isoWeek').subtract(1, 'days').format('YYYY-MM-DD');
// console.log("Last week's start date (Sunday):", lastWeekStartDate);

// Get the end of the last week (Saturday)
// Get the end of the last week (Saturday)
let lastWeekEndDate = moment().subtract(1, 'weeks').endOf('isoWeek').subtract(1, 'days').format('YYYY-MM-DD');
// console.log("Last week's end date (Saturday):", lastWeekEndDate);
     ...
     if(data.filter =="Last 3 Month"){
        // Get the start of the last week (Sunday)
        let last3MonthStartDate = moment().subtract(3, 'months').startOf('month').format('YYYY-MM-DD');
       ...
       if(data.filter =="Last 6 Month"){
        // Get the start of the last week (Sunday)
        let last6MonthStartDate = moment().subtract(6, 'months').startOf('month').format('YYYY-MM-DD');
       ...
       if(data.filter =="Last 12 Month"){
        // Get the start of the last week (Sunday)
        let last12MonthStartDate = moment().subtract(12, 'months').startOf('month').format('YYYY-MM-DD');
...
let previousYearStartDate = moment().subtract(1, 'years').startOf('year').format('YYYY-MM-DD');
// console.log("Previous year's start date:", previousYearStartDate);

// Get the end of the previous year
let previousYearEndDate = moment().subtract(1, 'years').endOf('year').format('YYYY-MM-DD');
